...
Adresat AdresaciMenedzer::podajDaneNowegoAdresata()//int idZalogowanegoUzytkownika, int idOstatniegoAdresata
{
    ...
    adresat.ustawId( (plikZAdresatami.pobierzIdOstatniegoAdresata() + 1 ) );//++idOstatniegoAdresata
    ...
    for (vector <Adresat>::iterator itr = adresaci.begin(); itr != adresaci.end(); itr++)
    {
        if (itr -> pobierzId() == idUsuwanegoAdresata)
        {
            ...
            if (znak == 't')
            {
                ...
                plikZAdresatami.usunWybranaLinieWPliku(idUsuwanegoAdresata);//numerLiniiUsuwanegoAdresata
                cout << "USUWANE ID: " << itr->pobierzId() << endl;
                adresaci.erase(itr);
...
int AdresaciMenedzer::wczytajLiczbeCalkowita()
{
    string wejscie = "";
    int liczba = 0;
...
void AdresaciMenedzer::zaktualizujDaneWybranegoAdresata(Adresat adresat)
{
    ...
    plikZAdresatami.edytujWybranegoAdresata(adresat.pobierzId(), liniaZDanymiAdresata);
